...
def fill_with_seed(win):
    ...
    while stack:
        ...
        y = y + 1
        while x <= xr:
            # ищем затравку на строки выше
            Fl = 0
            # пока пиксель != гран_значение and пиксель != нов_значение and x <= xr
            while win.image.pixel(x, y) != edge and  win.image.pixel(x, y) != fill and  x <= xr:
                if Fl == 0:
                    Fl = 1
                x = x + 1
            # помещаем в стек крайний справый пиксель
            if Fl == 1:
            ...
            if x == xt: # удостоверимся, что координата пиксела увеличена
                x = x + 1
